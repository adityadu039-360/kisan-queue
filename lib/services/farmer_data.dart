...
const List<String> procurementSteps = [
  'Process Started',
  'Quality Checked',
  'Price Confirmed',
  'Weight Confirmed',
...
class FarmerBooking {
  ...
  bool get isCompleted =>
      completedSteps.length == procurementSteps.length &&
          completedSteps.every((step) => step);

  int get completedCount =>
      completedSteps.where((step) => step).length;
  ...
  factory FarmerBooking.fromMap(
      Map<String, dynamic> map,
      ) {
    final rawSteps = map['completedSteps'];
    ...
    while (steps.length < procurementSteps.length) {
      steps.add(false);
    }

    if (steps.length > procurementSteps.length) {
      steps = steps.sublist(
        0,
...
class FarmerDataService extends ChangeNotifier {
  ...
  static const String _farmersKey =
      'farmers_data';

  static const String _bookingsKey =
      'farmer_bookings';

  static const String _cropsKey =
      'procurement_crops';
  ...
  Future<bool> updateBookingStep({
    required String token,
    required int stepIndex,
  }) async {
    if (stepIndex < 0 ||
        stepIndex >= procurementSteps.length) {
      return false;
    }
